...
function walk(answers = []) {
  ...
  for (const a of answers) {
    ...
    path_.push({ node: id, q: n.q, answer: a, matched: !!opt });
  ...
  return cur.terminal
    ? { path: path_, done: true, node: id, rule: cur.rule, exit: cur.exit, quote: cur.quote, derived: t.derived }
    : { path: path_, done: false, node: id, q: cur.q, why: cur.why,
...
function allQuestions() {
  ...
  return Object.entries(t.nodes)
    .filter(([, n]) => !n.terminal)
    .map(([id, n]) => ({ id, q: n.q, options: (n.options || []).map(o => o.a) }));
}
...
{"root":"<node id>","nodes":{
  "<id>":{"q":"<question to the trader>","why":"<one line: why the course asks it here>",
...
Rules: 12 nodes maximum; every "next" must name a node that exists; every path must end at a terminal node; every terminal must carry a verbatim course quote; include an option for "not visible on this chart" wherever the trader may be unable to tell.`;
...
async function refresh() {
  ...
  try {
    ...
    for (const [id, n] of Object.entries(t.nodes)) {
      ...
      if (!(n.options || []).length) bad.push(`${id}: question with no options`);
    }
