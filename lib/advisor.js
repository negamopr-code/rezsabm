...
async function start({ imageBase64, mime, notes = '', precise = false }) {
  ...
  publish();                                        // picture into the PDF; no NLM query yet
...
async function conclude(id) {
  ...
  try {
    ...
    const walkText = step.path.map((p, i) => `${i + 1}. ${p.q}\n   TRADER: ${p.answer}`).join('\n');
    const q = [
      ...
      `\nThe tree ends at: ${step.rule} — ${step.exit}`,
      ...
      `2. IS THE TREE'S RULE RIGHT — does "${step.rule}" hold given the trader's answers, or does the course prescribe something else for this exact combination? Quote the course.`,
      `3. THE EXIT — the concrete level or condition. The trader's answers are the facts of record; where you disagree, give the exit under HIS answers first, then say how it would change under yours.`,
    ...
    entry.walk = { path: step.path, rule: step.rule, exit: step.exit, quote: step.quote, reading: reading.answer };
    put(entry);
    publish();
    return entry;
  } finally {
