...
/** One question against the chart source + the SABM course.
 *  Pass a conversationId to continue the thread about the same chart (NLM keeps the context,
 *  and the transcript is written back into the PDF so the next upload carries it too). */
async function ask(question, sourceIds, { timeoutS = 300, conversationId = null } = {}) {
  ...
  let answer = '', err = '', conv = null, citations = null;
  try {
    ...
    citations = v.citations && Object.keys(v.citations).length ? v.citations : null;
  } catch {
  ...
  return { answer, conversationId: conv, citations };
}
